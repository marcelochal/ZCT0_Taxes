*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZARIV_RESP
*   generation date: 30.08.2018 at 12:47:35
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZARIV_RESP          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZARIV_RESP'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZARIV_RESP_EXTRACT
TOTAL = ZARIV_RESP_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZARIV_RESP'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZARIV_RESP-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZARIV_RESP_EXTRACT
TOTAL = ZARIV_RESP_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZARIV_RESP-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZARIV_RESP'
...
TOTAL = ZARIV_RESP_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZARIV_RESP-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZARIV_RESP-UPD_FLAG NE SPACE.
